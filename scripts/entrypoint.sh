...
# python -c "from eastern_state.cli import main; main()"
...
if [ -z ${EASTERN_STATE_BUCKET+x} ]; then
  ...
  source <(/usr/local/bin/eastern_state load_environment "$EASTERN_STATE_BUCKET" "$EASTERN_STATE_NAME" "$EASTERN_STATE_ENV")
fi
